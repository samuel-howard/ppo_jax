...
from visualise import create_vis
...
wandb.init(project="sam_ppo", 
           config=config,
           name=env_name+'-'+str(config['val_loss_coeff'])+'-'+datetime.datetime.now().strftime("%d.%m-%H:%M"))
...
for outer_iter in tqdm(range(n_outer_iters)):
    ...
    if outer_iter % eval_iter == 0:
        key, key_eval, key_render = jax.random.split(key, 3)
        ...
        # vis = create_vis(env, env_params, key_render, model_params, model, n_actions, eval_discount)
        # render_path = "./renders/"+str(experience)+".gif"
        # vis.animate(render_path, view=True)
        # wandb.log({"Render": wandb.Video(render_path)}, experience)
